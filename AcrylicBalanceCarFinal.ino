...
void loop() {
  ...
  //for compass
  float headingNeed = get_heading1(lat1, lon1, lat2, lon2);
  bool rightDir = false;
  bool object = false;
  //int timer = 25;
    ...
    while(1)
    {
      ...
      { 
        ...
        if(AngleAvg < 40 || AngleAvg > -40)
        {
          ...
          if (rightDir == false && (headingDegrees < (headingNeed - 5) || headingDegrees > (headingNeed + 5)) )
          {
            //object = true;
            Turn_Need = 60;           
          }
...
double get_heading1(double lat1, double lon1, double lat2, double lon2)  
{
  float pi = 3.14159;
   double dLat = (lat2-lat1)*(pi/180);
  double dLon = (lon2-lon1)*(pi/180);
  double lati1 = lat1*(pi/180);
  double lati2 = lat2*(pi/180);
 
  double y = sin(dLon) * cos(lati2);
  double x = (cos(lati1)*sin(lati2)) -
        (sin(lati1)*cos(lati2)*cos(dLon));
